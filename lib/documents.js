...
function Image(options) {
    ...
    var _naturalDimensions = null;
    var _dimensionsPromise = null;
    
    function getNaturalDimensions() {
        if (_naturalDimensions !== null) {
            return promises.resolve(_naturalDimensions);
        }
        
        if (_dimensionsPromise) {
            return _dimensionsPromise;
        }
        
        _dimensionsPromise = options.readImage().then(function(arrayBuffer) {
            var buffer = Buffer.from(arrayBuffer);
            var dims = imageDimensions.getImageDimensions(buffer);
            _naturalDimensions = dims || {width: null, height: null};
    ...
    var imageObj = {
        ...
        // Method to populate natural dimensions and isCropped
        _populateNaturalDimensions: function() {
            return getNaturalDimensions().then(function(dims) {
                if (dims && dims.width && dims.height) {
                    imageObj.naturalWidth = dims.width;
                    imageObj.naturalHeight = dims.height;
                    imageObj.isCropped = (imageObj.width && imageObj.width < dims.width) ||
                                        (imageObj.height && imageObj.height < dims.height);
                }
                return imageObj;
            });
        }
...
function Header(children, properties) {
    properties = properties || {};
    return {
        type: types.header,
        children: children,
        headerType: properties.headerType || "default",
        sectionIndex: properties.sectionIndex || 0
    };
}
...
function Footer(children, properties) {
    ...
    return {
        type: types.footer,
        children: children,
        footerType: properties.footerType || "default",
        sectionIndex: properties.sectionIndex || 0
    };
}
