...
function readStylesXmlWithTheme(root, theme) {
    ...
    // Also extract the Normal style defaults (often the real document defaults)
    var normalStyleDefaults = extractNormalStyleDefaults(paragraphStyles, theme);
    if (docDefaults && normalStyleDefaults) {
        docDefaults.normalStyle = normalStyleDefaults;
    }
    ...
    if (rPrDefault) {
        ...
        if (rPr) {
            ...
            if (fontSizeElement) {
                ...
                // Use Word's built-in default font size (12pt) when no explicit size is set
                defaults.character.fontSize = 12;
            }
...
function extractNormalStyleDefaults(paragraphStyles, theme) {
    // First try to find the default paragraph style by w:default="1" flag
    var defaultStyle = null;
    for (var styleId in paragraphStyles) {
        var style = paragraphStyles[styleId];
        if (style.element && style.element.attributes["w:default"] === "1" && style.element.attributes["w:type"] === "paragraph") {
            defaultStyle = style;
            break;
        }
    }
    
    // Fall back to "Normal" style if no w:default="1" found
    if (!defaultStyle) {
        defaultStyle = paragraphStyles["Normal"];
    }
    
    if (!defaultStyle || !defaultStyle.element) {
        return null;
    }

    var defaults = {};
    var rPr = defaultStyle.element.first("w:rPr");
    if (rPr) {
        defaults.character = {};
        ...
        if (fontElement) {
            ...
            // Determine primary font (same logic as document defaults)
            var primaryFont = null;
            if (defaults.character.font.hAnsi) {
                primaryFont = defaults.character.font.hAnsi;
            } else if (defaults.character.font.ascii) {
                primaryFont = defaults.character.font.ascii;
            } else if (theme && defaults.character.fontTheme) {
                if (defaults.character.fontTheme.hAnsi) {
                    primaryFont = resolveThemeFont(defaults.character.fontTheme.hAnsi, theme);
                } else if (defaults.character.fontTheme.ascii) {
                    primaryFont = resolveThemeFont(defaults.character.fontTheme.ascii, theme);
                }
            }
            defaults.character.primaryFont = primaryFont;
        }
