...
function BodyReader(options) {
    ...
    function readTableProperties(element) {
        ...
        if (tableBorders && !returnValue.isBordered) {
    ...
    function readDetailedBorders(bordersElement) {
        ...
        bordersElement.children.forEach(function(borderElement) {
            ...
            if (side && (side === 'top' || side === 'bottom' || side === 'left' || side === 'right')) {
                borders[side] = border;
            }
        });

        // Return null if no borders are found, otherwise return the borders object
        var hasBorders = borders.top || borders.right || borders.bottom || borders.left;
    ...
    function readBlip(element, blip) {
      ...
      // Get further formatting options
        ...
        // Extract cropping information from a:srcRect
        var srcRect = blip.firstOrEmpty("a:srcRect");
        var cropping = null;
        var isCropped = false;
...
function readNumberingProperties(styleId, element, numbering) {
    ...
    if (level !== undefined && numId !== undefined) {
        ...
        if (levelInfo) {
            var returnObject = Object.assign({}, levelInfo, {numId: numId});
            // Add numId to the returned level information
            return returnObject;
        }
        return null;
    }
